...
- Photoshop 中可以持续看到完整资产作为绘制参考。
- Blender 继续使用各自独立的项目贴图文件。
- 单贴图保存后可以刷新，不需要重新打开或重建材质。
- 多贴图写回前会验证关联文档与目标文件。
- 受支持的写回操作开始前，会为受影响贴图建立经过验证的备份。
- 当当前文件仍符合恢复记录时，**Undo Last Merge** 可以恢复最近一次成功写回。

这些保护用于补充正常的生产备份，不替代独立备份或版本管理。
...
通过官方销售渠道购买，可获得经过验证的正式发行包，以及销售页面所说明的支持或更新服务。

产品支持由实际购买平台处理。支付、收据、税务或 VAT、下载权限、退款和平台账户问题也由对应销售平台负责。

分享 Operation Report 或任务日志前，请移除私人路径、账户名、客户名、未公开美术引用、凭据、支付信息和机密生产数据。
